<template>
  <section class="min-h-screen flex items-stretch text-white">
    <div
      ...
      style="background-image: url(https://i.postimg.cc/CMNx3yQx/place.png)"
    >
      ...
      <div class="w-full px-24 z-10">
        ...
        <p class="text-3xl my-4">PT beIT Inovasi Tiwikrama</p>
      </div>
    </div>
    <div
    ...
    >
      ...
      <div class="w-full py-6 z-20">
        ...
        <p class="text-gray-100 font-semibold text-xl">Login to get in</p>
        <form action="" class="sm:w-2/3 w-full px-4 lg:px-0 mx-auto">
          <div class="pb-2 pt-4">
            <input
              ...
              class="text-black block w-full p-4 text-lg rounded-sm bg-gray-100"
            />
          </div>
          <div class="pb-2 pt-4">
            <input
          ...
          <div class="px-4 pb-2 pt-4">
            <div
              @click="login"
              class="uppercase block w-full p-4 text-lg rounded-full bg-indigo-500 hover:bg-indigo-600 focus:outline-none font-medium"
            >
              sign in
            </div>
          </div>
  ...
  props:[
    ...
    methods: {
        async login(){
            ...
            const post = await this.api(this.input.password, this.input.username)
            if(post.data.success){
            await this.$store.dispatch("authChange", post.data.data.api_token)
            console.log(post)
            this.$message({
              type: 'success',
            ...
            this.$message({
              type: 'warning',
              message: 'Please input login data'
            })
            return false;
          }
        }
